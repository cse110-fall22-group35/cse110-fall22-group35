  ...
  function localStorageDB (db_name, engine) {
    ...
    function alterTable (table_name, new_fields, default_values) {
      ...
      if (typeof default_values !== 'undefined') {
        ...
        for (const ID in db.data[table_name]) {
    ...
    function select (table_name, ids, start, limit, sort, distinct) {
      let ID = null; let results = []; let row = null;
    ...
    function sort_results (field, order) {
      return function (x, y) {
        ...
        const v1 = typeof (x[field]) === 'string' ? x[field].toLowerCase() : x[field];
        const v2 = typeof (y[field]) === 'string' ? y[field].toLowerCase() : y[field];
    ...
    function update (table_name, ids, update_function) {
      ...
      for (let i = 0; i < ids.length; i++) {
        ...
        const updated_data = update_function(clone(db.data[table_name][ID]));

        if (updated_data) {
          delete updated_data.ID; // no updates possible to ID

          const new_data = db.data[table_name][ID];
